...
func TestSummarizeProcessor(t *testing.T) {
	...
	if req.Messages[1].Role != llm.RoleSystem {
		t.Errorf("expected second message to be summary (system role), got %s", req.Messages[1].Role)
	}

	expectedSummary := "<conversation_summary>\nSummarized conversation\n</conversation_summary>"
	if req.Messages[1].Content != expectedSummary {
		t.Errorf("expected summary content '%s', got '%s'", expectedSummary, req.Messages[1].Content)
	}
